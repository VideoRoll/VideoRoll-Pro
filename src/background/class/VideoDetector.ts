...
export default class VideoDetector {
    ...
    chrome.webRequest.onSendHeaders.addListener(
      (details) => {
        ...
        if (requestHeaders) {
          ...
          if (isPlaylist) {
            currentTabId =
              requestHeaders.find((v) => v.name === "X-Extension-Tab-Id")
                ?.value ?? tabId;
          }
  ...
  async handleHLSContent(content: any, baseVideoInfo: any) {
    ...
    if (manifest.playlists && manifest.playlists.length > 0) {
      ...
      manifest.playlists.forEach((playlist) => {
        ...
        if (playlist.attributes?.CODECS) {
          const codecs = playlist.attributes.CODECS.split(",").map((codec) =>
            codec.trim()
          );
          // 使用Set去重，合并编解码器列表
    ...
    if (
      ...
      if (codecsMatch) {
        const codecs = codecsMatch[1].split(",").map((codec) => codec.trim());
      ...
      for (const item of qualities) {
        const response = await fetch(item.uri, {
          headers: {
            ...
            "X-Extension-Video-Kbps": Math.floor(item.bandwidth / 1000),
