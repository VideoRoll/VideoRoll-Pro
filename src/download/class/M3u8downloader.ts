...
export class M3U8Downloader {
    ...
    async parserM3u8() {
        ...
        try {
            const response = await fetch(this.url, {
                headers: this.videoInfo?.requestHeaders
            });
            if (!response.ok) {
    ...
    async fetchPlaylist(manifest: any) {
        // 找出最高质量的播放列表
        const highestQuality = manifest.playlists.reduce((prev, current) => {
            const prevBandwidth = prev.attributes?.BANDWIDTH || 0;
            const currentBandwidth = current.attributes?.BANDWIDTH || 0;
            return currentBandwidth > prevBandwidth ? current : prev;
        }, {});
        
        if(!highestQuality.uri.startsWith('https://') || !highestQuality.uri.startsWith('http://')) {
            this.url = new URL(highestQuality.uri, this.url).href
            // this.url = `${new URL(this.url).origin}${highestQuality.uri.startsWith('/') ? '' : '/'}${highestQuality.uri}`;
        } else {
            // 更新目标URL为最高质量的播放列表
            this.url = highestQuality.uri;
        }
    ...
    async downloadTs(segment: TSegmentType, index: number) {
        ...
        this.onProgress(this.progress, index + 1, this.segments.length, this.fileSize);
    ...
    mergeSegments() {
        ...
        this.finishSegments = validSegments;
    ...
    async transcodeToMp4ByMux(segments?: Uint8Array[]): Promise<Uint8Array[]> {
        ...
        if (!window.muxjs) return Promise.reject();
        if (!segments) {
            segments = this.segments
                .filter(
                    (segment) => segment.status === "success" && segment.data
                )
                .map((segment) => segment.data);
        }
    ...
    async getTSData() {
        return new Blob(this.finishSegments, { type: "video/mp2t" });
    }

    async getMP4Data() {
        const mp4Segments = await this.transcodeToMp4ByMux(this.finishSegments);
