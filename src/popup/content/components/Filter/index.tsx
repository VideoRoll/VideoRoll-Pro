...
export default defineComponent({
  ...
  setup() {
    ...
    const setFilter = (type: Exclude<keyof IFilter, "mode">, value: number) => {
    ...
    const popupRender = () => (
      <div class="video-roll-filter">
        ...
        >
          <van-radio name="unset">
            {browser.i18n.getMessage("video_unset")}
          </van-radio>
          <van-radio name="custom">
            {browser.i18n.getMessage("video_custom")}
          </van-radio>
        </van-radio-group>
        <div class="video-roll-filter-custom">
          {filterConfigs.map((item) => (
            <div class="video-roll-filter-slider">
              <div
                class={
                  filter.mode === "unset" ? "disabled-label" : "filter-label"
                }
              >
                {item.type}:{" "}
              </div>
              <van-stepper
                ...
                onUpdate:modelValue={(value: number) => {
                  setFilter(item.type as Exclude<keyof IFilter, "mode">, value);
                }}
              ></van-stepper>
            </div>
          ))}
    ...
    return () => (
      <div
        v-tooltip={browser.i18n.getMessage("video_filter")}
      ...
      >
        <div class="video-roll-icon-box">
          <span class="video-roll-label">
            {
              <ColorPaletteOutline class="video-roll-icon"></ColorPaletteOutline>
            }
          </span>
        </div>
      </div>
