...
export const shortcutsMap = {
    'Rotate ⭮': { title: getMessage('options_shortcuts_rotate_left'), key: 'deg', handler: (data: number) => { if (data > 0 && data + 90 >= 360) return 0; return data + 90; }, shortcuts: { key: '', code: [] } },
    'Rotate ⭯': { title: getMessage('options_shortcuts_rotate_right'), key: 'deg', handler: (data: number) => { if (data === 0 && data - 90 < 0) return 270; return data - 90; }, shortcuts: { key: '', code: [] } },
    'Screenshot': {
    ...
    'Muted': { title: getMessage('audio_muted'), key: 'muted', handler: (data: boolean) => { return !data }, shortcuts: { key: '', code: [] } },
    'Focus': { title: getMessage('video_focus'), key: 'focus', handler: (data: any) => { data.on = !data.on; return data }, shortcuts: { key: '', code: [] } },
    'Volume +': { title: getMessage('options_shortcut_volume_p'), key: 'volume', handler: (data: number) => { if (data + 1 <= 6) return data + 1; return data; }, shortcuts: { key: '', code: [] } },
    'Volume -': { title: getMessage('options_shortcut_volume_r'), key: 'volume', handler: (data: number) => { if (data - 0.1 >= 0) return data - 0.1; return data; }, shortcuts: { key: '', code: [] } },
    'PlaybackRate +': { title: getMessage('options_shortcut_speed_p'), key: 'playbackRate', handler: (data: number) => { if (data + 1 <= 16) return data + 1; return data; }, shortcuts: { key: '', code: [] } },
    'PlaybackRate -': { title: getMessage('options_shortcut_speed_r'), key: 'playbackRate', handler: (data: number) => { if (data - 0.5 >= 0) return data - 0.5; return data; }, shortcuts: { key: '', code: [] } },
